...
export class CartService {
  ...
  async changeQuantity(email: string, productData: Cart): Promise<CartResponse | null> {
    const buyer = await this.buyersService.getSingleBuyerByEmail(email);
    if (!buyer) return null;
  
    const { productId, quantity } = productData;
    if (!productId) throw new NotFoundException('Invalid product ID');
  
    const product = await this.productsRepository.findOne({ _id: productId });
    if (!product) throw new NotFoundException('Product not found');
  
    const existingItemIndex = buyer.cart.findIndex(
      (item) => item.productId._id.toString() === productId
    );
    ...
    if (existingItemIndex !== -1) {
      ...
      buyer.cart.push({ productId: product, quantity });
    }
  
    await buyer.save();
  
    const updatedCart = buyer.cart.map((item) => ({ product: item.productId, quantity: item.quantity }));
